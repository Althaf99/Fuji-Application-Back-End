...
public class StockQueryService {
    ...
    public PageResponse<StockRow> list(ItemType type, Long itemId, LocalDate from, LocalDate to,
        ...
        if (type == null || type == ItemType.rawMaterial)
            rawMaterials.findAll().stream().filter(i -> itemId == null || i.getId().equals(itemId))
                    .forEach(i -> add(rows, ItemType.rawMaterial, i.getId(), i.getCode(), start, end));
        if (type == null || type == ItemType.masterBatch)
            masterBatches.findAll().stream().filter(i -> itemId == null || i.getId().equals(itemId))
                    .forEach(i -> add(rows, ItemType.masterBatch, i.getId(), i.getCode(), start, end));
        if (lowStock != null)
    ...
    public StockRow get(ItemType type, Long id) {
        var result = list(type, id, null, null, null, 0, 1, "itemId", "asc");
        if (result.getContent().isEmpty())
            throw VendorService.missing("Stock item", id);
        return result.getContent().get(0);
    }

    private void add(List<StockRow> rows, ItemType type, Long id, String code, LocalDate from, LocalDate to) {
        var received = grnItems.sumQuantity(type, id, from, to);
        var consumed = consumptions.sumQuantity(type, id, from, to);
        rows.add(new StockRow(id, type, code, received, consumed, received.subtract(consumed),
